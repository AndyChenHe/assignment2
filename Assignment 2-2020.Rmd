...
label <- letters[1:10]
S3$breaks <- cut(S3$scores, breaks = 10, labels = label)
```
...
hist(S3$scores, col = S3$colors,xlab="SCORES",main = "Scores Frequence")
...
sample(interest,100,replace = TRUE)
S3$stid <- seq(1,100,1)
...
boxplot(S3$scores~S3$interest,col = interest.col)
...
logins <- sample(seq(1,25,1), 100, replace = TRUE)
S3$login <- logins
```
...
S3$interestColor <- factor(S3$interest)
levels(S3$interestColor) <- interest.col
S3$interestColor <- as.character(S3$interestColor)
plot(S3$login, S3$scores, col = S3$interestColor,pch=19,xlab = "Login times", ylab = 
       "Scores", main = "Scores VS Login times")
legend(
  "bottomright",legend = levels(factor(S3$interest)), col = levels(factor(S3$interestColor)), pch=19,trace=TRUE
)
...
```{r}
data("AirPassengers")
airPassengersData <- AirPassengers
class(airPassengersData)
plot(airPassengersData, ylab="Passengers (1000s)", type="o", pch =20)
airPassengersData
...
```{r}
DF3 <- DF2 %>% group_by(lesson_name,hash) %>% summarise(TotalAttempt = sum(attempt, na.rm = TRUE))
```
...
```{r}
DF5 <- DF4 %>% group_by(lesson_name,hash) %>% summarise(Mean = mean(correct, na.rm = TRUE))
```
...
```{r}
format(as.Date(swirlData$datetime, "%B %d %Y"), "%m/%d/%Y")
```
